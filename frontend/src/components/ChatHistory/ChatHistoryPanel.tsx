...
export function ChatHistoryPanel(_props: ChatHistoryPanelProps) {
  ...
  const clearAllDialogContentProps = {
    ...
    title: !clearingError ? 'Êtes-vous sûr de vouloir effacer tout l’historique des discussions ?' : 'Erreur lors de la suppression de tout l’historique des discussions',
    closeButtonAriaLabel: 'Close',
    subText: !clearingError
      ? 'Tout l’historique des discussions sera définitivement supprimé.'
      : 'Veuillez réessayer. Si le problème persiste, veuillez contacter l’administrateur du site.'
  }
  ...
  const menuItems: IContextualMenuItem[] = [
    { key: 'clearAll', text: ' effacer tout l’historique des discussions', iconProps: { iconName: 'Delete' } }
  ]
                  ...
                  <Stack horizontalAlign="center" verticalAlign="center" style={{ width: '100%', marginTop: 10 }}>
                    <StackItem>
                      <Text style={{ alignSelf: 'center', fontWeight: '400', fontSize: 16 }}>
                        ...
                        {!appStateContext?.state.isCosmosDBAvailable?.status && <span>Erreur lors du chargement de l'historique des discussions</span>}
                      </Text>
                    </StackItem>
                    <StackItem>
                      <Text style={{ alignSelf: 'center', fontWeight: '400', fontSize: 14 }}>
                        <span>L'historique des discussions ne peut pas être enregistré pour le moment</span>
                      </Text>
                    </StackItem>
                  </Stack>
                </Stack>
