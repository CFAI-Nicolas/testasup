...
import Contoso from '../../assets/Logo.png'
...
const Chat = () => {
  ...
  const [answerId, setAnswerId] = useState<string>('')
    ...
    useEffect(() => {
        if (
            ...
            let subtitle = `${appStateContext.state.isCosmosDBAvailable.status}. Veuillez contacter l'administrateur du site.`
            setErrorMsg({
                title: 'L historique des discussions n est pas activé',
                subtitle: subtitle
            })
            toggleErrorDialog()
        }
  ...
  const makeApiRequestWithCosmosDB = async (question: ChatMessage["content"], conversationId?: string) => {
    ...
    let result = {} as ChatResponse
      var errorResponseMessage = 'Veuillez réessayer. Si le problème persiste, veuillez contacter l administrateur du site.'
    try {
      ...
      if (!response?.ok) {
        ...
        let errorChatMsg: ChatMessage = {
          id: uuid(),
          role: ERROR,
            content: `Une erreur s'est produite lors de la génération d'une réponse. L'historique des discussions ne peut pas être enregistré pour le moment. ${errorResponseMessage}`,
          date: new Date().toISOString()
        }
    ...
    const clearChat = async () => {
        ...
        if (appStateContext?.state.currentChat?.id && appStateContext?.state.isCosmosDBAvailable.cosmosDB) {
            ...
            if (!response.ok) {
                setErrorMsg({
                    title: 'Erreur lors de la suppression de la discussion en cours',
                    subtitle: 'Veuillez réessayer. Si le problème persiste, veuillez contacter l administrateur du site.'
                })
                toggleErrorDialog()
            } else {
    ...
    const tryGetRaiPrettyError = (errorMessage: string) => {
        try {
            ...
            if (match) {
                ...
                if (reason !== '') {
                    return (
                        'L’invite a été filtrée en raison du déclenchement du système de filtrage de contenu d’Azure OpenAI.\n' +
                        'Raison : cette invite contient du contenu marqué comme ' +
                        reason +
                        '\n\n' +
                        'Veuillez modifier votre message et réessayer. En savoir plus : https://go.microsoft.com/fwlink/?linkid=2198766'
                    )
                }
            }
        } catch (e) {
        ...
        if (!noContentError) {
          saveToDB(appStateContext.state.currentChat.messages, appStateContext.state.currentChat.id)
            .then(res => {
              if (!res.ok) {
                let errorMessage =
                  "Une erreur s'est produite. Les réponses ne peuvent pas être enregistrées pour le moment. Si le problème persiste, veuillez contacter l'administrateur du site."
                let errorChatMsg: ChatMessage = {
                  id: uuid(),
                  role: ERROR,
                  content: errorMessage,
    ...
    <div className={styles.container} role="main">
      {showAuthMessage ? (
        <Stack className={styles.chatEmptyState}>
          ...
          />
        <h1 className={styles.chatEmptyStateTitle}>Authentification non configurée</h1>
          <h2 className={styles.chatEmptyStateSubtitle}>
                      Cette application n'a pas d'authentification configurée. Veuillez ajouter un fournisseur d'identité en trouvant votre application dans le{' '}
          ...
          <h2 className={styles.chatEmptyStateSubtitle} style={{ fontSize: '20px' }}>
                      <strong>La configuration de l'authentification prend quelques minutes à appliquer. </strong>
          </h2>
          <h2 className={styles.chatEmptyStateSubtitle} style={{ fontSize: '20px' }}>
                      <strong>Si vous avez effectué un déploiement au cours des 10 dernières minutes, veuillez patienter et recharger la page après 10 minutes.</strong>
          </h2>
        </Stack>
      ) : (
            ...
            <Stack.Item className={styles.citationPanel} tabIndex={0} role="tabpanel" aria-label="Citations Panel">
              <Stack
                ...
                <IconButton
                  ...
                  aria-label="Fermer les citations"
          ...
          {messages && messages.length > 0 && isIntentsPanelOpen && (
            <Stack.Item className={styles.citationPanel} tabIndex={0} role="tabpanel" aria-label="Intents Panel">
              <Stack
                ...
                <IconButton
                  iconProps={{ iconName: 'Cancel' }}
                  aria-label="Fermer les intentions"
